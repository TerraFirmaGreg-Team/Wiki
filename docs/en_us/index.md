---
layout: home

hero:
  name: TerraFirmaGreg Wiki
  tagline: Survival Modpack As It Should've Been
  actions:
    - theme: brand
      text: Download on CurseForge
      link: https://www.curseforge.com/members/terrafirmagreg/projects
    - theme: alt
      text: Download on Modrinth
      link: https://modrinth.com/organization/terrafirmagreg
    - theme: alt
      text: Latest Changelog
      link: /en_us/major-changelogs/0.12
    - theme: alt
      text: Official Site
      link: https://terrafirmagreg.team

features:
  - title: Upgrade Guides
    details: Step-by-step instructions for migrating your worlds between major versions.
    link: /en_us/upgrade-guides/from-0.11-to-0.12
    linkText: 0.11 → 0.12
  - title: Major Release Changelogs
    details: Highlights of new content, mechanics, and notable changes per major release.
    link: /en_us/major-changelogs/0.12
    linkText: Read 0.12 changelog
  - title: Developer Information
    details: KubeJS, Datagen, multiblock scripts, Pixel Composer, worldgen — everything contributors need.
    link: /en_us/developer/cheat-sheet
    linkText: Open the cheat sheet
  - title: Optional Mods
    details: Compatible mods that aren't bundled by default but pair well with TFG.
    link: /en_us/modpack/optional-mods
    linkText: See the list
---

Want us to add more content, or just want to hang out? Join us on [Discord](https://discord.com/invite/AEaCzCTUwQ).
