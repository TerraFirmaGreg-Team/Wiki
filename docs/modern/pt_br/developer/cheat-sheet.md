...
```
* .itemIds
Isso acessa a lista de IDs de itens dessa tag de ingrediente. O resultado é um Java Set.
* .toArray()
...
O exemplo abaixo é usado no builder de trim de armadura.
```js
...
## Imprimindo JSONs de Receita no Log
...
```js
	event.findRecipes({ id: string }).forEach(recipe => {
		// Obtém o JsonObject "inputs" existente
		const inputs = recipe.json.get("inputs");

		// Obtém o array de itens atual ou cria um novo
		const itemArray = inputs.has("item") ? Java.from(inputs.get("item")) : [];

		// Adiciona o JSON de entrada do circuito
		itemArray.push({
			content: {
				type: "gtceu:circuit",
				configuration: 1
		...
		// Define o array de itens atualizado de volta em inputs
		inputs.add("item", itemArray);

		// Reaplica os inputs modificados no JSON da receita
		recipe.json.add("inputs", inputs);
	});
```
Você também pode sobrescrever os inputs diretamente; isso removerá quaisquer outros inputs existentes:
```js
	event.findRecipes({ id: string }).forEach(recipe => {
...
```
	* Você pode substituir `{id: string}` por qualquer método de receita do KubeJS; como `{type: string}` ou `{mod: string}`, por exemplo.
	...
	## Entrada/Saída com Chance
	Como escrever ingredientes de entrada/saída com chance para receitas do GTCEu.
```js
...
Para obter o caminho de remap de um mixin, clique com o botão direito no método no IntelliJ e use Copy Special > Mixin Target Reference.
...
Se você quiser gerar uma receita para uma máquina do GregTech _sem_ que o Greate gere uma receita (por exemplo, uma receita de bender que você não quer na mech press), coloque `_electric_only` ou `_manual_only` no final do ID da receita.
...
# Firmalife
Você pode fazer planters crescerem instantaneamente clicando com o botão direito neles com um bloco de bedrock.
