...
---
## 速查表
本页面的目的是供开发者记录有用的函数，以备将来使用。如果您有任何值得分享的 JavaScript 或 Java 代码，请放在这里。
...
```js
	event.findRecipes({ id: string }).forEach(recipe => {
		// 获取现有的 "inputs" JsonObject
		const inputs = recipe.json.get("inputs");

		// 获取当前的 item 数组或创建一个新的
		...
		// 将更新后的 item 数组设置回 inputs 中
		inputs.add("item", itemArray);

		// 将修改后的 inputs 重新应用到配方 JSON 中
...
如果您的 mixin 覆盖了 Minecraft 中的任何内容，或者注入到了一个从 Minecraft 继承的方法中，您的 mixin 需要 `remap = true`。其他所有情况都应使用 `remap = false`。
